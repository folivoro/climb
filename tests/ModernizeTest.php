...
function writeClimbConfig(string $tmpDir): void
{
    file_put_contents($tmpDir . '/climb-config.json', json_encode([]));
}
...
describe('prerequisites', function () {
    ...
    it('fails when climb-config.json is missing', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);

        $this->artisan('modernize')
            ->expectsOutputToContain('climb-config.json')
            ->expectsOutputToContain('wp sloth dump-config')
            ->assertFailed();
...
describe('version detection via installed.php', function () {
    ...
    it('detects folivoro/sloth v1 as v1', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
    ...
    it('detects dev-main as v1', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
    ...
    it('detects 9999999-dev as v1', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
    ...
    it('warns and aborts on ambiguous dev branch without --from', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
    ...
    it('accepts --from to override ambiguous dev branch', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
        writeInstalledPhp($tmpDir, 'folivoro/sloth', 'dev-feat/my-feature');
        ...
        $this->artisan('modernize', ['--from' => 1])
            ->expectsOutputToContain('v1 → v2')
            ->assertSuccessful();
    ...
    it('fails when no Sloth package is found', function () {
        ...
        $this->artisan('modernize')
            ->expectsOutputToContain('No Sloth package found')
            ->assertFailed();
...
describe('version detection fallback via composer.lock', function () {
    it('falls back to composer.lock when installed.php is absent', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
        // No installed.php
...
describe('downgrade warning', function () {
    it('warns when installed version >= target and aborts on no', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
    ...
    it('proceeds when user confirms downgrade', function () {
        ...
        $this->artisan('modernize', ['--from' => 1, '--to' => 2])
            ->expectsOutputToContain('downgrade')
...
describe('happy path', function () {
    it('calls setup, run and report on the modernizer in order', function () {
        $tmpDir = commandSetup();
        writeComposerJson($tmpDir);
        writeClimbConfig($tmpDir);
        writeInstalledPhp($tmpDir, 'sixmonkey/sloth', '1.0.0');
